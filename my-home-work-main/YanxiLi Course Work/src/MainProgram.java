...
public class MainProgram {
    ...
    private static void ticketInformation() {
        ...
        if (listOfTicket.isEmpty()) {
            ...
            for (Ticket ticket1 :listOfTicket) {
                int ticketNumber = ticket1.getAvailableNumber();
                System.out.println("Tickets for these shows: " + ticket1.getShowName() +" is available");
                System.out.println("Tickets' availableNumber: " + ticketNumber);
            ...
            for (Member member : listOfMember) {
                ...
                if (member.getTickets().isEmpty()) {
                    ...
                    for (String ticketName : member.getTickets().keySet()) {
                        int quantity = ticketName.split(" ").length  - 1;
    ...
    private static void buyTickets() {
        ...
        if (quantity < 0 || quantity >= listOfTicket.size()) {
            ...
            ticket.setAvailableNumber(ticket.getAvailableNumber() - quantity);
            System.out.println("You have successfully bought this ticket!");
        }else {
            System.out.println("Failed to buy ticket. Because you already have 3 types of tickets or you already buy this ticket.");
        }
    }
    ...
    private static void cancelTickets() {
        ...
        if (ticketIndex< 0 || ticketIndex >= ticketNames.size()) {
            System.out.println("Please enter a valid Ticket index");
            return;
        }

        String selectedTicket = ticketNames.get(ticketIndex);
        Ticket ticket = listOfTicket.get(ticketIndex);
        ...
        if(cancelQuantity <= quantity && cancelQuantity >= 0) {
            ...
            ticket.setAvailableNumber(ticket.getAvailableNumber() + cancelQuantity);
    ...
    /**
     * Write a letter as letters.txt
     *
     *
     */
    public static void writeLetter(Member member, Ticket ticket) {
            try (FileWriter letter = new FileWriter("letters.txt", false)) {
                PrintWriter writer = new PrintWriter(letter);
                ...
                writer.println();
                writer.println("Please select another show or enter a valid number");
                writer.println();
                writer.println("Sincerely,");
                writer.println("NEAT");
            } catch (IOException e) {
                System.out.println("Error writing to letters.txt. Please try again.");
        }
    }
}
